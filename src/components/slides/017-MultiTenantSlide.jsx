      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-[1fr_auto_1fr_auto_1fr] gap-4 items-center mb-8">
          ...
          <div className={`${t.cardBg} p-6 rounded-xl border ${t.cardBorder} text-center`}>
            ...
            <div className="text-sm text-slate-400">Define once, maintain once</div>
            <div className={`mt-3 px-3 py-1 rounded-full ${t.accentBg}/20 text-sm ${t.accentColor}`}>
          ...
          <div className={`${t.cardBg} p-6 rounded-xl border ${t.cardBorder} text-center`}>
            ...
            <div className="text-sm text-slate-400 mb-3">Auto-deploy to all tenants</div>
            <div className="flex flex-wrap gap-2 justify-center">
              {tenants.map((tenant, i) => (
                <div key={i} className={`${tenant.color} px-2 py-1 rounded text-xs font-medium text-white`}>
                  {tenant.name}
                </div>
              ))}
            </div>
          ...
          <div className={`${t.cardBg} p-6 rounded-xl border ${t.cardBorder} text-center`}>
            ...
            <div className="text-sm text-slate-400">Aggregated results, per-client context</div>
            <div className={`mt-3 px-3 py-1 rounded-full bg-emerald-500/20 text-sm text-emerald-400`}>
        ...
        <div className={`${t.cardBg} p-6 rounded-xl border ${t.cardBorder}`}>
          <div className="text-lg font-bold text-slate-200 mb-4 text-center">Trigger Options</div>
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-4 rounded-lg bg-slate-800/50">
              <div className={`text-2xl font-bold ${t.accentColor}`}>Scheduled</div>
              <div className="text-sm text-slate-400 mt-1">Daily/weekly scans</div>
            </div>
            <div className="text-center p-4 rounded-lg bg-slate-800/50">
              <div className={`text-2xl font-bold ${t.accentColor}`}>Webhook</div>
              <div className="text-sm text-slate-400 mt-1">On-demand from tickets</div>
            </div>
            <div className="text-center p-4 rounded-lg bg-slate-800/50">
              <div className={`text-2xl font-bold ${t.accentColor}`}>Event-Driven</div>
              <div className="text-sm text-slate-400 mt-1">On config change</div>
            </div>
          </div>
        </div>
