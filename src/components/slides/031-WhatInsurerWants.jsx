      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className={`${t.cardBg} p-5 rounded-xl border border-red-500/30 mb-6`}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <div className="text-2xl font-bold text-red-400">Why Claims Get Denied</div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            {denialReasons.map((item, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-red-500/10">
                ...
                <div>
                  <div className="text-red-400 font-bold text-xl">{item.reason}</div>
                  <div className="text-slate-400 text-xl mt-1">{item.example}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {checklist.map((section, i) => (
            <div key={i} className={`${t.cardBg} p-4 rounded-xl border ${t.cardBorder}`}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <div className={`font-bold text-xl ${i === 2 ? 'text-amber-400' : 'text-slate-200'}`}>
                  {section.category}
                </div>
              </div>
